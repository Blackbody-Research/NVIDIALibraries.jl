#=*
* Check if CUDA runtime v10.1 functions are wrapped properly.
*
...
@test (cudaIpcCloseMemHandle(C_NULL) == cudaErrorInvalidValue)
...
println("cudaStreamBeginCapture(): ", unsafe_string(cudaGetErrorName(cudaStreamBeginCapture(cudaStream_t(C_NULL), cudaStreamCaptureMode(255)))))
@test (cudaStreamBeginCapture(cudaStream_t(C_NULL), cudaStreamCaptureMode(255)) == cudaErrorInvalidValue)

c_array = zeros(cudaStreamCaptureMode, 1)
println("cudaThreadExchangeStreamCaptureMode(): ", unsafe_string(cudaGetErrorName(cudaThreadExchangeStreamCaptureMode(c_array))))
@test (cudaThreadExchangeStreamCaptureMode(c_array) == cudaSuccess)
